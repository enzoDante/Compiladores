...
llvm::Value* CodeContext::genExpr(ExprNode* node) {
    ...
    if (auto* n = dynamic_cast<VarExpr*>(node)) {
        ...
        return builder.CreateLoad(builder.getInt32Ty(), it->second, n->name);
    }
    ...
    if (auto* n = dynamic_cast<BinaryOp*>(node)) {
        ...
        if (n->op == "==") return builder.CreateICmpEQ(l, r, "eq");
        if (n->op == "&&") return builder.CreateAnd(l, r, "and");
        if (n->op == "||") return builder.CreateOr(l, r, "or");
    }
