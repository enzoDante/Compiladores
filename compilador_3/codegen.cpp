...
llvm::Value* CodeContext::genExpr(ExprNode* node) {
    ...
    if (auto* n = dynamic_cast<BinaryOp*>(node)) {
        ...
        if (n->op == "+")  return builder.CreateAdd(l, r, "add");
        if (n->op == "-")  return builder.CreateSub(l, r, "sub");
        if (n->op == "*")  return builder.CreateMul(l, r, "mul");
        if (n->op == "/")  return builder.CreateSDiv(l, r, "div");
        if (n->op == "==") {
            // garante que ambos são i32 para comparar
            auto toI32 = [&](llvm::Value* v) -> llvm::Value* {
                if (v->getType() == builder.getInt1Ty())
                    return builder.CreateZExt(v, builder.getInt32Ty(), "zext");
                return v;
            };
            return builder.CreateICmpEQ(toI32(l), toI32(r), "eq");
        }
        if (n->op == "&&") {
            ...
            return builder.CreateAnd(toBool(l), toBool(r), "and");
        }
        if (n->op == "||") {
            auto toBool = [&](llvm::Value* v) -> llvm::Value* {
                if (v->getType() == builder.getInt1Ty()) return v;
            ...
            return builder.CreateOr(toBool(l), toBool(r), "or");
        }
        if (n->op == "<")  return builder.CreateICmpSLT(l, r, "lt");
        if (n->op == ">")  return builder.CreateICmpSGT(l, r, "gt");
        if (n->op == "<=") return builder.CreateICmpSLE(l, r, "lte");
        if (n->op == ">=") return builder.CreateICmpSGE(l, r, "gte");
        if (n->op == "!=") return builder.CreateICmpNE(l, r,  "neq");
    }
